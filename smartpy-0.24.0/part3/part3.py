...
def main():
    ...
    class PokeContract(sp.Contract):
        def __init__(self, initial_message: sp.string, admin: sp.address):
        ...
        def poke_me_back(self, passed_ticket: poke_ticket):
            with sp.modify_record(self.data) as data:
              ...
              match contract_opt:
                  case Some(contract):
                      sp.transfer(data.feedback, sp.mutez(0), contract)
                  case None:
...
def test():
    ...
    contract = main.PokeContract(initial_message="Hello", admin=admin.address)
    scenario += contract
    ...
    contract2 = main.PokeContract(initial_message="You poked me!", admin=admin.address)
    scenario += contract2
