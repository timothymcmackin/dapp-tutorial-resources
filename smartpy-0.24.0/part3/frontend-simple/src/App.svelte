<script>
  ...
  const myPokeContract = "KT1WQqnmHnU1jRzY8ySxREPeNNbZHN38dLUs"; // Part 2
  ...
  const pokeOtherContract = async (sourceAddress, targetAddress) => {
    ...
    try {
      const op = await sourceContract.methodsObject.poke_other_contract(targetAddress).send();
      console.log(`Waiting for ${op.opHash} to be confirmed...`);
      await op.confirmation(2);
      await getContracts();
    } catch (error) {
  ...
  const createTicket = async (contractAddress) => {
    const contract = await Tezos.wallet.at(contractAddress);
    try {
      const op = await contract.methodsObject
        .create_ticket(address)
        .send();
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          ...
          <button on:click={() => createTicket(oneContract.address)}> Create ticket (admin only)</button>
        </td>
      </tr>
      {/each}
    </tbody>
